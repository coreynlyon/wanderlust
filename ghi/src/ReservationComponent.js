...
export default function Reservation() {
  ...
  const handleDelete = (id) => async () => {
    try {
      const url = `${process.env.REACT_APP_TRIP_SERVICE_API_HOST}/reservations/${id}`;
    ...
    return (
      <Paper shadow="sm" p="xl">
        ...
        {reservation ? (
          <>
            <Text>
              {reservation.accommodation_name} (Confirmation #:{" "}
              {reservation.reservation_num})
              <br /> {reservation.address}
              <br />
              <br /> Check in:{" "}
              {new Date(reservation.check_in).toLocaleString("en-US", {
                year: "numeric",
                month: "numeric",
                day: "numeric",
                hour: "2-digit",
              ...
              <br /> Check out:{" "}
              {new Date(reservation.check_out).toLocaleString("en-US", {
                year: "numeric",
                month: "numeric",
                day: "numeric",
                hour: "2-digit",
